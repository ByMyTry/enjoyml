...
params_to_pooling = {
    2: {
        MAX_MODE: layers.MaxPool1D,
        AVERAGE_MODE: layers.AvgPool1D,
    },
    3: {
    ...
    4: {
        MAX_MODE: layers.MaxPool3D,
        AVERAGE_MODE: layers.AvgPool3D,
    },
}
...
class FixedPooling2D(layers.Layer):
    ...
    def _get_base_pool_layer(self, input_size, output_size):
        ...
        PoolClassLayer = params_to_pooling[self.dimension][self.pool_mode]
        ...
        while row_steps > 0 or col_steps > 0:
            if row_steps > 0:
                ...
                pooled_tensor_row = K.expand_dims(
                    reduce_func([tensor[:, i, :, :],
                                 tensor[:, i + 1, :, :]], axis=0),
                    axis=1
                )
                tensor = K.concatenate([tensor[:, :i, :, :],
                                        pooled_tensor_row,
                                        tensor[:, (i + 2):, :, :]], axis=1)
                row_steps -= 1
            if col_steps > 0:
                ...
                pooled_tensor_col = K.expand_dims(
                    reduce_func([tensor[:, :, j, :],
                                 tensor[:, :, j + 1, :]], axis=0),
                    axis=2
                )
                tensor = K.concatenate([tensor[:, :, :j, :],
                                       pooled_tensor_col,
                                       tensor[:, :, (j + 2):, :]], axis=2)
                col_steps -= 1
        return tensor
    ...
    def build(self, input_shape):
        ...
        if self.dimension < 1 or 3 < self.dimension:
            raise Exception(f'[{self.__class__.__name__}] Invalid dimension: dimension must be in: [1, 3].')
    ...
    def call(self, tensor, **kwargs):
        ...
        row_reduce_steps = base_pooled_tensor.get_shape()[-3] - self.output_size[0]
        col_reduce_steps = base_pooled_tensor.get_shape()[-2] - self.output_size[1]
        # result_shape_plh = K.placeholder(shape=[None, self.output_size[0], self.output_size[1],
        #                                         base_pooled_tensor.get_shape()[-1]])
        # result = K.zeros_like(result_shape_plh)
        result = None
        for iter_n in range(self.iterations):
            pooled_tensor = K.identity(base_pooled_tensor)
            pooled_tensor = self._random_reduce(pooled_tensor, seed_up=iter_n,
                                                row_steps=row_reduce_steps, col_steps=col_reduce_steps)
            if result is None:
